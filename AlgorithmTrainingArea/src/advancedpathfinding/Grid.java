...
public class Grid {
    ...
    public Grid(int mapSize, int fieldSize, int displaySize) {
        ...
        this.cSize = displaySize / mapSize;
    ...
    public void setUnit(Unit u) {
        int i = (int) (u.getX() / cSize);
        int j = (int) (u.getY() / cSize);
        ...
        for (int i = x; i < x + u.getSize(); i++) {
            for (int j = y; j < y + u.getSize(); j++) {
                if ((!MovingManager.isIgnore(map[i][j].getUnit(), u))
                        || (map[i][j].getGroundValue() < u.getSize())) {
                    return false;
                }
            }
        }
    ...
    public boolean isIgnoreWalkable(Unit u) {
        double ux = u.getX() + cSize * Math.cos(u.getAngle()) / 2;
        double uy = u.getY() + cSize * Math.sin(u.getAngle()) / 2;
        int x = (int) (ux / cSize);
        int y = (int) (uy / cSize);
        for (int i = x; i < x + u.getSize(); i++) {
            for (int j = y; j < y + u.getSize(); j++) {
    ...
    public boolean isAbsWalkable(Unit unit) {
        double x = unit.getX() + cSize * Math.cos(unit.getAngle()) / 2;
        double y = unit.getY() + cSize * Math.sin(unit.getAngle()) / 2;
        ...
        for (int i = 0; i < map.length; i++) {
            ...
            for (int j = 0; j < map.length; j++) {
                ...
                if (map[i][j].getGroundValue() < level) {
                    ...
                    g2d.fillRect(i * cSize, j * cSize, cSize, cSize);
                }
                g2d.setColor(Color.BLACK);
                g2d.drawRect(i * cSize, j * cSize, cSize, cSize);
            }
        }
    }
