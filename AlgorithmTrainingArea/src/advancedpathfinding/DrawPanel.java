...
public class DrawPanel extends JComponent implements Runnable {
    ...
    public DrawPanel() {
        super();
        u = new Unit(12 * grid.cSize + grid.cSize / 2, 12 * grid.cSize + grid.cSize / 2, 1);
        gu = new Unit(11 * grid.cSize + grid.cSize / 2, 12 * grid.cSize + grid.cSize / 2, 1);
        //u = new Unit(12 * grid.cSize + grid.cSize / 2, 12 * grid.cSize + grid.cSize / 2, 1);
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (aim != null) {
            ...
            g2d.fillRect(aim.x * grid.cSize - 1, aim.y * grid.cSize - 1, grid.cSize + 2, grid.cSize + 2);
        }
        ...
        while (curCell != null) {
            ...
            g2d.drawRect(curCell.x * grid.cSize + 2, curCell.y * grid.cSize + 2, grid.cSize - 4, grid.cSize - 4);
        ...
        while (curLeaf != null) {
            ...
            if (curLeaf.getLeaf() != null) {
                ...
                if (manager.getLeaf() != null) {
                    ...
                    c = Algorithm.searchPath(x, y, x0, y0);
                    while (c != null) {
                        pen1 = new BasicStroke(1);
                        g2d.setStroke(pen1);
                        g2d.setColor(Color.RED);
                        g2d.drawRect(c.x * grid.cSize + 2, c.y * grid.cSize + 2, grid.cSize - 4, grid.cSize - 4);
                        c = c.getCell();
                    }
                }
                ...
                g2d.drawLine(curLeaf.getLeaf().node.x * grid.cSize + grid.cSize / 2,
                        curLeaf.getLeaf().node.y * grid.cSize + grid.cSize / 2,
                        tmp.node.x * grid.cSize + grid.cSize / 2,
                        tmp.node.y * grid.cSize + grid.cSize / 2);
            }
